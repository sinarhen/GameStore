      ...
      <div className="flex sm:flex-row mb-5 justify-between items-center">
            ...
            >
              <Button onClick={() => {
                setSelectedCategory(category._id);
                      ...
                      className={cn("rounded-3xl bg-white text-black hover:bg-gray-300 border border-transparent", selectedCategory === category._id ? "bg-gray-400 border-indigo-600" : "")}>{category.name}</Button>
            </motion.div>
          ))}
        ...
        {isFilterApplied && (
          ...
          <p onClick={clearFilters}
             className={"text-sm items-center hover:bg-gray-400 sm:hover:bg-transparent rounded-full p-8 sm:bg-transparent sm:rounded-none sm:p-0 bg-gray-300 cursor-pointer text-gray-600 absolute right-2 bottom-4 z-40 hover:underline sm:relative justify-center gap-x-2 w-max"}>
