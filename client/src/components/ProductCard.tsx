...
export default function ProductCard({ product, ...props }: { props?: any; product: ProductCardType; }) {
    ...
    return (
        <div onClick={() => window.location.replace(`/products/${product._id}`)} className="max-w-md h- transition-all hover:translate-x-0.5 hover:-translate-y-0.5 group hover:bg bg-neutral-800 cursor-pointer mx-auto rounded-md overflow-hidden shadow-lg hover:shadow-xl">
