...
export default function CartItem({item}: {
    ...
    return(
        ...
        <div className="flex gap-x-2">
            <div onClick={(e) => {
            ...
            }}  className="aspect-square relative min-w-20 h-20 w-20 rounded overflow-hidden border border-transparent  group-hover:border-indigo-600 transition-all cursor-pointer">
                <span className="opacity-0 flex items-center justify-center group-hover:opacity-80 transition-all w-full h-full bg-black absolute">
                        <Check className={cn("text-green-600 absolute text-opacity-75 transition-opacity h-1/2 w-1/2 opacity-0", inputValue !== 0 ? "opacity-100" : "")}/>
                        <Trash2 className={cn("text-red-600 text-opacity-75 h-1/2 transition-opacity w-1/2 opacity-0", inputValue === 0 ? "opacity-100" : "")} />
                </span>
                <img src={item.productId.imageUrl} className="bg-cover object-cover"/>
            </div>
            <div className="flex flex-col justify-between">
                <div onClick={() => {navigate('/products/' + item.productId._id); setOpen(false)}} className="group-hover:text-indigo-600 transition-colors">
        ...
        <div className="mt-1 flex flex-col justify-between group-hover:text-indigo-600 transition-colors">
            <p className="text-xs ">{formatter.format(item.productId.price * item.quantity)}</p>
        
            <div className="h-full  group-hover:opacity-100 flex flex-col items-end justify-end opacity-0 transition-opacity ">
